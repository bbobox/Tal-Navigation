  ...
  */
/* variable.c */
/* Comparaison, intersection, soustraction d etiquettes de variables */
...
static tCode * traduit_symbole_en_code(tSymbole * s) {
  ...
  /* Ne modifie pas le symbole donne en entree. */
  ...
  if (1 + u_strlen(s->gramm) > maxGramm) {
    ...
    printf("code grammatical de longueur %d\n", u_strlen(s->gramm)) ;
    i_fprintf(stdout, "(%S)\n", s->gramm) ;
    return 0;
  }
...
static tCode * traduit_symbole_en_code_orig(tSymbole * s) {
