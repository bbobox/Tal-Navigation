...
grf_t * grf_load(FILE * f) {
  ...
  for (int i = 0; i < nb_boxes; i++) {
    ...
    while ((len == (bufsize - 1)) && (buf[len - 1] != '\n')) {  /* make sure we read all the line */
      ...
      bufsize = 2 * bufsize;
      buf = (unichar *) realloc(buf, bufsize);
      p = buf + len;
...
void grf_dump(grf_t * grf, FILE * f) {
  ...
  for (int i = 0; i < grf->nb_boxes; i++) {
    ...
    grf_box * box = grf->boxes[i];

    label = (char *) realloc(label, u_strlen(box->label) + 1);
    u_to_char(label, box->label);

    fprintf(f, "* %d: \"%s\" -- %dx%d\n trans: ", i, label, box->x, box->y);

    fputs("< ", f);

    for (grf_trans * trans = box->trans; trans; trans = trans->next) { fprintf(f, "%d, ", trans->to); }
...
/* reorder boxes according to newpos[] (must be a permutation of [0 .. nb_boxes -1])


void grf_reorder(grf_t * grf, int * newpos) {

  grf_box ** newboxes = (grf_box **) malloc(grf->nb_boxes * sizeof(grf_box *));
