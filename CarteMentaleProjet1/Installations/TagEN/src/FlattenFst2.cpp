...
printf("Flattening...\n%s",CR);
int result=flatten_main_graph(origin,depth,new_main_graph,RTN);
...
determinisation_new_main_graph(res,new_main_graph->states);
if (RTN) {
   save_graphs_to_keep(origin,res);
}
printf("Saving tags...\n%s",CR);
copy_tags_into_file(origin,res);
u_fclose(res);
// liberation of the dependences structures
...
for (int etat=grammar->debut_graphe_fst2[n];etat<limite;etat++) {
   ...
   while (trans!=NULL) {
      if (trans->etiquette<0) {
         ...
         *L=inserer_dans_liste_nombres(-(trans->etiquette),*L);
      }
      trans=trans->suivant;
   }
}
...
for (int i=1;i<=grammar->nombre_graphes;i++) {
   if (dependences[i]!=NULL) {
      printf("graph %d %s calls:\n%s",i,grammar->nom_graphe[i],CR);
      struct liste_nombres* l=dependences[i];
      while (l!=NULL) {
         printf("   graph %d %s\n%s",l->n,grammar->nom_graphe[l->n],CR);
         l=l->suivant;
      }
   }
}
...
for (int i=grammar->debut_graphe_fst2[1];i<limite;i++) {
    ...
    while (l!=NULL) {
       ...
       temp->arr=0+(l->arr)-grammar->debut_graphe_fst2[1];
...
for (int i=0;i<pos_in_tab;i++) {
   ...
   int starting_pos=flatten_sub_graph_recursively(grammar,-(tab[i]->etiq),1,max_depth,new_main_graph,tab[i]->arr,RTN,&SUBGRAPH_CALL_IGNORED,&SUBGRAPH_CALL);
...
for (int i=grammar->debut_graphe_fst2[1];i<limite;i++) {
    ...
    struct transition_fst* l=new_main_graph->states[i]->trans;
    while (l!=NULL) {
        printf("%d %d ",l->etiquette,l->arr);
        l=l->suivant;
    }
    printf("\n");
}
...
for (int i=grammar->debut_graphe_fst2[N];i<limite;i++) {
    ...
    while (l!=NULL) {
       if (RTN || (l->etiquette>=0 || (depth<max_depth))) {
          ...
          // and add to it the starting position of the current graph
          // which is 0 for the main graph
          temp->arr=initial_pos+(l->arr)-grammar->debut_graphe_fst2[N];
          temp->suivant=etat->trans;
          if ((temp->etiq) < 0) {
             // if the transition is a reference to a sub-graph
             if (depth<max_depth) {
                ...
                temp->etiq=-(new_graph_number[-(l->etiquette)]);
                (*SUBGRAPH_CALL)++;
             }
          }
          etat->trans=temp;
...
//
// this function determinises and saves the graph passed in parameter
//
...
for (int i=grammar->debut_graphe_fst2[N];i<limite;i++) {
   ...
   while (l!=NULL) {
      if (l->etiquette < 0) {
         ...
         sprintf(temp,"%d %d ",l->etiquette,(l->arr)-grammar->debut_graphe_fst2[N]);
      }
      u_fprints_char(temp,f);
      l=l->suivant;
   }
