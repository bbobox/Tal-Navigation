...
void usage() {
printf("%s",COPYRIGHT);
printf("Usage: Fst2Txt <text> <fst2> <alphabet> <MODE> [-char_by_char]\n%s",CR);
...
printf("Applying %s in %s mode...\n%s",argv[2],(MODE==MERGE)?"merge":"replace",CR);
...
int l=fread(buffer+N,sizeof(unichar),origine_courante,f);
origine_courante=0;
LENGTH=N+l;
}
...
void parse_text() {
LENGTH=fread(buffer,sizeof(unichar),BUFFER_SIZE,f);
origine_courante=0;
N=2;
...
if (etat_courant->controle&1) {
   ...
   if (pos>=taille_entree/*sommet>u_strlen(output)*/) {
      // and if the recognized output is longer than the current one, it replaces it
      pile[sommet]='\0';
...
if (THAI_MODE ||
   ...
   if (position!=0 &&
       ...
       int SOMMET2=sommet;
       struct liste_nbre* RES=get_matching_etiquettes(mot,arbre_etiquettes[e],alphabet,THAI_MODE);
...
inserer_etiquette(fst2->etiquette[trans->etiquette]->contenu,trans->etiquette,trans->arr,racine);
return tmp;
}
