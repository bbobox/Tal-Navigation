...
void create_graph(int ligne_courante,unichar** ligne,int n_champs,struct graphe_patron* g,
                  char* nom_resultat,/*char* chemin,*/FILE *f_coord) {
char nom_res[1000];
//char nom[1000];
...
if (ligne_courante%10==0) {
   printf("%d table entries done...                %s",ligne_courante,CR);
}
...
while (read_table_line(table,(unichar**)ligne,n_champs)) {
  create_graph(ligne_courante,ligne,n_champs,&structure,subgraph,/*chemin,*/result_graph);
  ligne_courante++;
}
u_fclose(table);
