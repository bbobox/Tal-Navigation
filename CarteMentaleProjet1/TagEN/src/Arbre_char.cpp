...
void explorer_arbre(unichar* texte,int pos,struct arbre_char* noeud,Alphabet* alphabet,int THAI_MODE,int max_pos) {
if (noeud==NULL) {
   return;
}
if (THAI_MODE) {
...
while (trans!=NULL) {
  if (is_equal_or_case_equal(trans->c,texte[pos],alphabet)) {
     ...
     explorer_arbre(texte,pos+1,trans->noeud,alphabet,THAI_MODE,max_pos);
  }
  trans=trans->suivant;
}
}
...
struct liste_nbre* get_matching_etiquettes(unichar* texte,
                                           ...
                                           Alphabet* alphabet,int THAI_MODE) {
RES=NULL;
explorer_arbre(texte,0,racine,alphabet,THAI_MODE,0);
return RES;
}
