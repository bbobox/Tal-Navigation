...
void chargeAlphabet(FILE * fich, char * nomFich, tAlphMot * alphabet) {
   ...
   alphabet->symb = (tSymbole *) calloc(alphabet->nbSymboles, sizeof(tSymbole)) ;

   if (! alphabet->symb)
     erreurMem("chargeAlphabet") ;
...
static tTransitions ** copie_tab_etats(tTransitions ** tabOrig, etat nbEtats) { 
  ...
  tabNouv = (tTransitions **)calloc(1, sizeof(tTransitions *) * nbEtats); 

  if (! tabNouv) erreurMem("copie_tab_etats") ;
...
static char * copie_tab_sorte(char *tabOrig,etat nbEtats) { 
  ...
  nouvTab=(char*)calloc(1, sizeof(char)*nbEtats);

  if (! nouvTab) erreurMem("copie_tab_sorte") ;
...
nouvTab = (etat *) calloc(1,  sizeof(etat) * nbElements) ; 
if (! nouvTab)
   erreurMem("copie_tab_initial") ;
...
void remplitSymbole(tSymbole * s, tSymbole * source) {
  ...
  case NEGATIF : {
    s -> canonique = (unichar *) calloc(1 + u_strlen(source -> canonique), sizeof(unichar)) ;
  
  if (! s -> canonique)
      erreurMem("remplitSymbole") ;
  ...
  case CODE : case INCOMPLET : {
    ...
    if (! s->canonique)  erreurMem("copieSymbole") ;
...
/*void Enregistre_Dico(FILE * f, tSymbole alphabet[], int nbSymboles,
   BOOL chevrons) {
   int n = 0 ;
fprintf(f,"\n%%") ;
for (n=0;n < nbSymboles ;n++) {
   if (chevrons)
      fprintf(f, "<") ;
   switch (alphabet[n].sorteSymbole) {
      case ATOME :
         fprintf(f, "%s,%s.%s",
            alphabet[n].flechie, alphabet[n].canonique, alphabet[n].gramm) ;
         break ;
      case UNIVERSEL :
         fprintf(f, ".") ;
         break ;
      case CODE : case INCOMPLET :
         fprintf(f, ".%s", alphabet[n].gramm) ;
         break ;
      case NEGATIF :
         fprintf(f, "%s.%s", alphabet[n].canonique, alphabet[n].gramm) ;
         break ;
      default :
         printf("Erreur interne [Enregistre_Dico].\n") ; }
   if (chevrons)
      fprintf(f, ">%%") ;
   else fprintf(f, "%%") ; }
}*/
...
/*int Affiche_Automate(tAutAlMot * Aut)
{
int i,lig=2,last=20;
tTransitions * t;
printf ("\n Affichage de l'automate %p\n",Aut); fflush(stdout);
printf ("\n Nombre d'etats : %d",Aut->nbEtats); fflush(stdout);
  for (i=0;i<Aut->nbEtats;i++)
  {
  printf ("\n ----------------  etat %d  : sorte : %d",i+1,Aut->sorte[i]); fflush(stdout);
  lig++;
  t=Aut->etats[i];
	while (t)
	{
	lig+=Affiche_Transition(t);
	if (lig>=last) { getchar(); last+=20; }
  /@* Attend toutes les 20 lignes *@/
	t=t->suivant;
	}
  }
return lig;
...
/*static int Affiche_Transition(tTransitions * t)
{
printf ("\n    Transition vers etat %d\n",t->but+1); fflush(stdout);
return Affiche_Symbole(t->etiq)+1;
}*/
...
void autalmot_dump_fst2(tAutAlMot * A, FILE * f) {
  ...
  for (etat i = 0; i < A->nbEtats; i++) {
	...
	case INDETERMINE:
	  error("ouptut_fst: symbol code INDETERMINE\n");
...
void autalmot_resize(tAutAlMot * a, int size) {
  ...
  if (size < a->nbEtats) { die("autalmot_resize: size(=%d) < nbEtats(=%d)\n", size, a->nbEtats); }
