...
void sentence_to_grf(Automate_fst2* automate,int SENTENCE,FILE* f) {
...
convertir_transitions_en_etats(automate,SENTENCE,nombre_etats,rang,f,nombre_etats_res,rang_max,width_max,pos_X);
free(rang);
free(pos_X);
}
...
// and we save the grf
save_grf_states(f,tab_grf_state,N_GRF_STATES,rang_max);
}
...
write_grf_header(width_max+300,800,N,f);
...
void write_grf_header(int width,int height,int N,FILE* f) {
unichar z[10];
...
write_grf_header(tab_grf_state[1]->pos_X+300,800,N_GRF_STATES,f);
u_fprints_char("\"<E>\" 50 100 ",f);
// we save the initial state
int j=0;
l=tab_grf_state[0]->l;
